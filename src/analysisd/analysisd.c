...
/* Reported mutexes */
/* Per-output-file writer mutexes — split from the former single            */
/* writer_threads_mutex to allow parallel writes to different output types.  */
/* Deadlock rule: always acquire in order archives→alerts→firewall→fts.      */
static pthread_mutex_t wr_mutex_archives = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t wr_mutex_alerts   = PTHREAD_MUTEX_INITIALIZER;  /* shared by alerts + statistical writers (both use _aflog/_jflog) */
static pthread_mutex_t wr_mutex_firewall = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t wr_mutex_fts      = PTHREAD_MUTEX_INITIALIZER;
    ...
    while (1) {
        if (recv = OS_RecvUnix(m_queue, OS_MAXSTR, buffer), recv) {
            ...
            if (msg[0] == SYSCHECK_MQ) {
                ...
                if (result == -1) {
                    drop_buffer_persist(buffer, (size_t)recv);
...
void * w_writer_thread(__attribute__((unused)) void * args ){
    ...
    while(1){
        ...
        if (lf = queue_pop_ex(writer_queue), lf) {
            ...
            w_mutex_lock(&wr_mutex_archives);
            ...
            Free_Eventinfo(lf);
            w_mutex_unlock(&wr_mutex_archives);
        }
    }
}
...
#endif
            w_mutex_unlock(&wr_mutex_alerts);
            Free_Eventinfo(lf);
        }
    }
}
...
void * w_log_rotate_thread(__attribute__((unused)) void * args){
    ...
    while(1){
        ...
        /* Acquire all output mutexes in fixed order before flush+rotation.
         * Individual writer threads only ever hold ONE mutex, so no deadlock. */
        w_mutex_lock(&wr_mutex_archives);
        w_mutex_lock(&wr_mutex_alerts);
        w_mutex_lock(&wr_mutex_firewall);
        w_mutex_lock(&wr_mutex_fts);
        ...
        w_mutex_unlock(&wr_mutex_fts);
        w_mutex_unlock(&wr_mutex_firewall);
        w_mutex_unlock(&wr_mutex_alerts);
        w_mutex_unlock(&wr_mutex_archives);
        sleep(1);
    }
...
void * w_writer_log_statistical_thread(__attribute__((unused)) void * args ){
    ...
    while(1){
        ...
        if (lf = queue_pop_ex(writer_queue_log_statistical), lf) {
            ...
            w_mutex_lock(&wr_mutex_alerts);  /* statistical shares _aflog/_jflog with alerts */
            w_inc_stats_written();
