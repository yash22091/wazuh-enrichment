...
typedef struct _remoted_state_t {
    ...
    uint64_t analysisd_queue_drop_count; /**< Events dropped because DEFAULTQUEUE was full/broken */
} remoted_state_t;
...
/**
 * @brief Increment counter for events dropped because DEFAULTQUEUE is full
 *        or SendMSG failed even after reconnect.
 */
