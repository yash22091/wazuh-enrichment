...
int rem_write_state() {
    ...
    fprintf(fp,
        ...
        "\n"
        "# Events dropped forwarding to analysisd (DEFAULTQUEUE full/broken)\n"
        "analysisd_queue_drop_count='%llu'\n"
        "\n"
        ...
        state_cpy.analysisd_queue_drop_count,
...
void rem_inc_analysisd_queue_drop(void) {
    w_mutex_lock(&state_mutex);
    remoted_state.analysisd_queue_drop_count++;
    w_mutex_unlock(&state_mutex);
}
...
cJSON* rem_create_state_json() {
    ...
    cJSON_AddNumberToObject(_received_breakdown, "analysisd_queue_drops", state_cpy.analysisd_queue_drop_count);
